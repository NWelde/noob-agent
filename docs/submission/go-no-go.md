...
| A fresh clone into the scratchpad plus the README quickstart succeeds, and `uv run pytest` passes | **Partially.** Confirmed reproducible in `docs/reproducibility.md`; on this branch (= `origin/main`) `uv run pytest -q` gives `1 failed, 597 passed, 5 skipped` — the 1 failure is the section-26.3 test that PR #76 fixes | Agent (re-verify after #76 merges) — requester does the actual fresh clone |
